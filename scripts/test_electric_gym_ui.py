...
from test_oranienburg_gym import badge,tm_count
...
if __name__=='__main__':
    ...
    try:
        for state,expected in [('electric-gym-entrance',0),('electric-gym-complete',1)]:
            ...
            assert [emu.read(card+17+i,1) for i in range(8)]==[1,1,expected,0,0,0,0,0]
            emu.screenshot(ROOT/f'test-output/{state}-card.png')
            emu.press('B',180);emu.press('B',90)
            assert emu.location()[:2]==(43,26)
        emu.state(ROOT/'test-output/electric-gym-complete.state',True)
        open_key_item(emu,361)
        wait_task(emu,'Task_EuropeMap')
        assert emu.read('sEuropeMapCurrent',1)==5
        emu.press('B',180);emu.press('B',180);emu.press('B',90)
        assert not emu.read('sLockFieldControls',1)
        assert emu.location()==(43,26,6,6)
        emu.state(ROOT/'test-output/electric-gym-complete.state',True)
        open_key_item(emu,364);emu.frames(180)
        assert emu.read('sTMCaseDynamicResources')!=0
        assert tm_count(emu)==1
        emu.screenshot(ROOT/'test-output/electric-gym-tm-case.png')
        emu.press('B',180);emu.press('B',180);emu.press('B',90)
        assert emu.location()==(43,26,6,6) and badge(emu)
        assert not emu.read('sLockFieldControls',1)
        print('PASS: Trainer Card shows all three earned badges; Gym map shows Oranienburg; awarded TM opens in TM Case',flush=True)
    finally: emu.close()
