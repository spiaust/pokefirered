...
from test_oxford_gym import badge,tm_count
...
if __name__=='__main__':
    ...
    try:
        for state,expected in [('gym-entrance',0),('gym-complete',1)]:
            ...
            assert [emu.read(card+17+i,1) for i in range(8)]==[expected,0,0,0,0,0,0,0]
            emu.screenshot(ROOT/f'test-output/{state}-card.png')
            emu.press('B',180);emu.press('B',90)
            assert emu.location()[:2]==(43,24)
        emu.state(ROOT/'test-output/gym-complete.state',True)
        open_key_item(emu,361)
        wait_task(emu,'Task_EuropeMap')
        assert emu.read('sEuropeMapCurrent',1)==3
        emu.press('B',180);emu.press('B',180);emu.press('B',90)
        assert not emu.read('sLockFieldControls',1)
        assert emu.location()==(43,24,6,6)
        emu.state(ROOT/'test-output/gym-complete.state',True)
        open_key_item(emu,364);emu.frames(180)
        assert emu.read('sTMCaseDynamicResources')!=0
        assert tm_count(emu)==1
        emu.screenshot(ROOT/'test-output/gym-tm-case.png')
        emu.press('B',180);emu.press('B',180);emu.press('B',90)
        assert emu.location()==(43,24,6,6) and badge(emu)
        assert not emu.read('sLockFieldControls',1)
        print('PASS: Trainer Card shows exactly the first badge; Gym map shows Oxford; awarded TM opens in TM Case',flush=True)
    finally: emu.close()
