...
for city in sys.argv[1:] or ['Amiens','Rouen']:
 id=f'LAYOUT_EUROPE_{city.upper()}_PAST';l=layouts[id];w,h=l['width'],l['height'];ow=24 if city=='Amiens' else 36
 assert sum(m['layout']==id for m in maps)==1
 old=struct.unpack('<%dH'%(ow*20),(R/f'data/geography/{city.lower()}-v055.bin').read_bytes());new=struct.unpack('<%dH'%(w*h),(R/l['blockdata_filepath']).read_bytes())
