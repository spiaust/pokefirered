...
DATA={'BeauvaisGarden':(32,4,[(49,9,'cathedral'),(38,12,'palace'),(31,26,'west-bridge'),(55,26,'east-bridge'),(45,30,'river')],[(49,13,'UP'),(38,15,'UP'),(46,24,'UP')])}

def run(city):
 index,selection,points,signs=DATA[city]
 layout=next(l for l in json.loads((ROOT/'data/layouts/layouts.json').read_text())['layouts'] if l.get('id')=='LAYOUT_EUROPE_BEAUVAIS_GARDEN')
 ...
 a=(ROOT/'data/tilesets/primary/general/metatile_attributes.bin').read_bytes()+(ROOT/'data/tilesets/secondary/europe_beauvais/metatile_attributes.bin').read_bytes()
 def valid(x,y):
 ...
 try:
  load_checkpoint(e,'garden-active',True);checkmap(e);before=preserved(e)[1:];story=tuple(e.var(v) for v in range(0x40d5,0x40ef))
 ...
 try:
  ...
  from test_garden import leave,enter
  leave(e);enter(e);checkmap(e)
  from test_time import cross
  cross(e)
  assert e.location()==(43,17,15,24)
  print('PASS: Beauvais district Save/cold Continue, historical map, reception round trip and Celebi return',flush=True)
 finally:e.close()
