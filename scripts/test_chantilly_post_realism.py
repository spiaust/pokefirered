"""Walk historical Chantilly and preserve its original refuge story."""
...
DATA={'ChantillyPastPost':(30,4,[(36,10,'station'),(44,5,'tracks'),(25,18,'forecourt'),(28,26,'approach')],[(37,11,'UP'),(26,19,'LEFT'),(34,27,'UP')])}

def run(city):
 index,selection,points,signs=DATA[city]
 layout=next(l for l in json.loads((ROOT/'data/layouts/layouts.json').read_text())['layouts'] if l.get('id')=='LAYOUT_EUROPE_CHANTILLY_PAST_POST')
 W,H=layout['width'],layout['height'];tiles=struct.unpack('<%dH'%(W*H),(ROOT/layout['blockdata_filepath']).read_bytes());m=json.loads((ROOT/f'data/maps/Europe{city}/map.json').read_text());blocked={(o['x'],o['y']) for o in m['object_events']}
 a=(ROOT/'data/tilesets/primary/general/metatile_attributes.bin').read_bytes()+(ROOT/'data/tilesets/secondary/europe_chantillypost/metatile_attributes.bin').read_bytes()
 def valid(x,y):
 ...
 try:
  load_checkpoint(e,'post-v054-extended',True);checkmap(e);before=preserved(e)[1:];story=tuple(e.var(v) for v in range(0x40d5,0x40ef))
 ...
 try:
  ...
  go(e,(5,10));checkmap(e)
  from test_departure import back_to_refuge,visit_post
  back_to_refuge(e);visit_post(e);checkmap(e)
  print('PASS: station district Save/cold Continue, historical map and refuge round trip',flush=True)
 finally:e.close()
