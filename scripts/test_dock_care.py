"""Le Havre care: dock-task gate, healing, persistence and return routes."""
import argparse
from emulator import Emulator, ROOT
from test_celebi import load_checkpoint
from test_dock_check import worker, finish_check
from test_le_havre import PORT, captain, clerk, finish_port
...
def rest(emu, choice='YES'):
    ...
    emu.screenshot(ROOT / 'test-output/dock-care-offer.png')
...
if __name__ == '__main__':
    ...
    try:
        ...
        assert emu.var(0x40DA) == 1 and health(emu) == tired
        finish_check(emu); assert health(emu) == tired
        emu.state(ROOT / 'test-output/dock-care-tired.state')
        rest(emu, 'NO'); rest(emu, 'B'); rest(emu); rest(emu)
        emu.state(ROOT / 'test-output/dock-care-rested.state')
        inspect(emu, 49, 32767, 'dock-care')
        captain(emu, 'YES'); clerk(emu); tire_party(emu); rest(emu)
        cross(emu); finish_port(emu); tire_party(emu); rest(emu)
        print('PASS: dock-task gate, no automatic healing, No/B, repeat care and both return routes', flush=True)
        load_checkpoint(emu, 'dock-check-complete', args.legacy)
        tire_party(emu); rest(emu)
        print('PASS: existing completed dock-task save unlocks care immediately', flush=True)
