"""Walk coastal districts, old-save tiles, signs, saves, ferry and coach exits."""
...
DATA={
 'DoverPort':(27,6,[(45,9,'castle'),(48,13,'clifftop'),(53,17,'cliffs'),(35,23,'seafront'),(35,32,'harbor')],[(44,13,'UP'),(52,18,'RIGHT'),(29,23,'UP')]),
 'CalaisPort':(28,7,[(37,30,'hall'),(45,16,'lighthouse'),(35,10,'seafront'),(51,7,'pier'),(27,27,'gardens')],[(40,31,'UP'),(47,17,'UP'),(30,12,'UP')]),
}

def run(city):
 index,selection,points,signs=DATA[city]
 layout=next(l for l in json.loads((ROOT/'data/layouts/layouts.json').read_text())['layouts'] if l.get('id')==f'LAYOUT_EUROPE_{city[:-4].upper()}_PORT')
 ...
 try:
  load_checkpoint(e,'coast-'+city[:-4].lower(),True);checkmap(e);before=preserved(e)[1:]
 ...
 try:
  ...
  crossing(e);assert e.location()[1]==55-index
  crossing(e);assert e.location()==(43,index,8,5)
  checkmap(e);leave_port(e);assert e.location()[1]==(0 if index==27 else 4)
  print(f'PASS: {city} district Save/cold Continue, map return, ferry round trip and coach exit',flush=True)
 finally:e.close()
