...
DATA={'ChantillyPast':(29,4,[(35,11,'chateau'),(39,9,'moat'),(22,19,'stables'),(47,8,'gardens'),(28,26,'pelouse')],[(37,15,'UP'),(26,19,'LEFT'),(34,27,'UP')])}

def run(city):
 index,selection,points,signs=DATA[city]
 layout=next(l for l in json.loads((ROOT/'data/layouts/layouts.json').read_text())['layouts'] if l.get('id')=='LAYOUT_EUROPE_CHANTILLY_PAST')
 ...
 a=(ROOT/'data/tilesets/primary/general/metatile_attributes.bin').read_bytes()+(ROOT/'data/tilesets/secondary/europe_chantilly/metatile_attributes.bin').read_bytes()
 def valid(x,y):
 ...
 try:
  load_checkpoint(e,'time-arrival',True);checkmap(e);before=preserved(e)[1:];story=tuple(e.var(v) for v in range(0x40d5,0x40ef))
 ...
 try:
  ...
  cross(e);cross(e);checkmap(e)
  child(e);keeper(e);child(e)
  assert e.var(0x40ed)==4
  print('PASS: historical estate Save/cold Continue, era map, Celebi round trip and original blanket quest',flush=True)
 finally:e.close()
