"""Verify the Southampton terminal and original harbor art survive expansion."""
from pathlib import Path
import json,struct
from PIL import Image
R=Path(__file__).resolve().parents[1]
layouts={v.get('id'):v for v in json.loads((R/'data/layouts/layouts.json').read_text())['layouts']}
l=layouts['LAYOUT_EUROPE_SOUTHAMPTON_PAST'];w,h=l['width'],l['height'];assert (w,h)==(64,40)
...
base=R/'data/tilesets/secondary/island_harbor';new=R/'data/tilesets/secondary/europe_southamptonpast'
...
print('PASS: Southampton original save positions, tiles and palettes preserved; three-cell terminal bypass; layout fits hardware')
