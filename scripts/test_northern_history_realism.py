"""Walk Amiens and Rouen landmarks while preserving historical progression."""
...
DATA={
'AmiensPast':(33,4,[(49,20,'cathedral'),(35,6,'saint-leu'),(31,14,'west-bridge'),(54,9,'east-bridge'),(40,6,'canal')],[(50,24,'UP'),(35,9,'UP'),(45,14,'UP')]),
'RouenPast':(34,4,[(70,10,'cathedral'),(52,11,'clock'),(44,29,'west-bridge'),(72,24,'east-bridge'),(61,30,'seine')],[(70,15,'UP'),(53,14,'UP'),(60,24,'UP')]),
}

def run(city):
 index,selection,points,signs=DATA[city]
 layout=next(l for l in json.loads((ROOT/'data/layouts/layouts.json').read_text())['layouts'] if l.get('id')==f'LAYOUT_EUROPE_{city[:-4].upper()}_PAST')
 ...
 a=(ROOT/'data/tilesets/primary/general/metatile_attributes.bin').read_bytes()+(ROOT/f'data/tilesets/secondary/europe_{city[:-4].lower()}/metatile_attributes.bin').read_bytes()
 def valid(x,y):
 ...
 try:
  load_checkpoint(e,city[:-4].lower()+'-arrival',True);checkmap(e);before=preserved(e)[1:];story=tuple(e.var(v) for v in range(0x40d5,0x40ef))
 ...
 try:
  ...
  go(e,(10,16));checkmap(e)
  if city=='AmiensPast':
   from test_amiens import leave,board
  else:
   from test_rouen import leave,board
  leave(e);board(e);checkmap(e)
  from test_time import cross
  cross(e);assert e.location()==(43,17,15,24)
  print(f'PASS: {city} district Save/cold Continue, era map, train round trip and Celebi return',flush=True)
 finally:e.close()
